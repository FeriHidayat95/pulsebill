...
const RadiusManager = {
    ...
    /**
     * SAVE PROFILE (Radius Logic - Sultan Flexible Mode)
    ...
    async deletePppoeUser(customerId, adminUsername, req) {
        ...
        try {
            ...
            // =========================================================
            // ??? 4. TENDANG USER JIKA SEDANG ONLINE (VERSI SULTAN v7)
            // =========================================================
            if (username) {
    ...
    async expireUser(customerId, adminUsername, req) {
        ...
        try {
            ...
            // =========================================================
            // ??? 4. PROSES TENDANG (KICK) - VERSI SULTAN MELEK DATA
            // =========================================================
    ...
    async disconnectSession(username) {
        try {
            // ?? SULTAN FIX: Ambil data IP NAS, Secret, IP User (Framed-IP), dan MAC User (Calling-Station-Id)
            ...
            if (session.length > 0) {
                ...
                return new Promise((resolve) => {
                    // Pastikan modul exec sudah di-require di paling atas file Bos
                    const { exec } = require('child_process'); 
    ...
    async getActiveHotspotUsers() {
        // Query Sakti Bos: Mengambil data radacct + Join NAS + Subquery Profile
        const sql = `
            SELECT 
                r.radacctid as id, 
                r.username as name, 
        ...
        // Mapping Data sesuai format asli Bos
        return rows.map(row => ({
            ...row,
    ...
    /**
     * 2. KICK USER HOTSPOT (VERSI SULTAN - ANTI MAMPET)
    ...
    async getDetailedHotspotProfiles() {
        ...
        rows.forEach(row => {
            ...
            // Pemetaan atribut sesuai logika asli Bos
    ...
    /**
     * SULTAN SEARCH ENGINE (API VOUCHER HISTORY)
    ...
    async generateHotspotVouchers(d) {
        ...
        try {
            ...
            // 2. LOGIKA FALLBACK (Nurut Kode Asli Bos)
            ...
            // Helper Generator Kode (Sesuai Logika Asli Bos)
            const generateCode = () => {
                let str = '';
    ...
    async addUser(d) {
        ...
        try {
            ...
            // 3. LOGIKA FALLBACK (Nurut Kode Asli Bos): 
    ...
    /**
     * UPDATE MEMBER FULL (SULTAN SYNC ENGINE v10 - ABSOLUTE DETAIL)
    ...
    async updateMemberFull(d) {
        ...
        try {
            ...
            return { success: true, message: "Update Sultan Berhasil! Waktu dihitung segar dari detik ini." };
    ...
    async deleteSingleVoucher(username) {
        ...
        try {
            ...
            // =========================================================
            // ?? 2. BERSIHKAN DATABASE (Sesuai keinginan Bos: Bersih Total)
            // =========================================================
            await conn.beginTransaction();
    ...
    async deleteVouchersBatch(usernames) {
        ...
        try {
            ...
            // =========================================================
            // ?? LANGKAH 2: BERSIHKAN TOTAL (Sesuai Keinginan Bos)
            // =========================================================
            await conn.beginTransaction();
     ...
     * VERSI V8 - SULTAN ABSOLUTE PRECISION (FIX TGL PASANG & 34 HARI)
     */
    async addHotspotMember(d) {
        // SOLDERAN SULTAN: Tangkap data sesuai payload frontend
        ...
        try {
            ...
            // --- 1. CLEANING DATA (WhatsApp Sultan Tetap Aman) ---
            ...
            await conn.commit();
            console.log(`? [SULTAN V8] ${username} Berhasil! Tgl Pasang: OK | Detik: ${selisihDetik} (34 Hari)`);
     ...
     * SULTAN VERSION - ANTI BOCOR PPPOE
     */
    async getVoucherMonitorData(filters) {
        const { profile = '', search = '', status = '' } = filters;

        // ??? KLEP FILTER SULTAN: Hanya ambil profil yang BUKAN PPP
        const hotspotFilter = `
            rg.groupname NOT IN (
            ...
            AND rg.groupname NOT LIKE 'VLAN%' -- Jaga-jaga profil VLAN Bos
            AND rg.groupname NOT LIKE 'PAKET%' -- Jaga-jaga profil PAKET Bos
        `;
    ...
    async deleteBulkMonitorVouchers(selectedVouchers) {
        ...
        try {
            ...
            // =========================================================
            // ?? 2. JALANKAN OPERASI SAPU BERSIH (Sesuai Instruksi Bos)
            // =========================================================
            await conn.beginTransaction();
