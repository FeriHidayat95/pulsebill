...
class InvoiceScheduler {
    ...
    async initScheduler() {
        ...
        // Ini adalah "Algojo" utama. Pastikan fungsi di serviceSuspension sudah kita update kuerinya.
        ...
        logger.info('?? Scheduler Sultan Aktif - Semua Operasi Dijalankan Sebelum Subuh.');
    }
    
    async sendDueDateReminders() {
        try { // <-- INI YANG HILANG DI KODE BOS
    ...
    // --- OTOT ALGOJO: GENERATE TAGIHAN BULANAN (SULTAN RADAR CYCLE) ---
    async generateMonthlyInvoices() {
        // 1. GEMBOK ANTI-RACE CONDITION (Mencegah mesin jalan ganda)
        if (this.isGenerating) {
            logger.info("[ALGOJO] Mesin generate masih bekerja, menolak eksekusi ganda.");
            return 0;
        }
        ...
        try {
            ...
            logger.info(`[ALGOJO] Patroli Radar H-${hDays} dimulai...`);
                ...
                try {
                    ...
                    for (const cycle of cyclesToProcess) {
                        ...
                        if (existing.length > 0) {
                            logger.info(`[ALGOJO] Skip ${customer.name}, tagihan untuk tanggal ${dueStr} sudah ada.`);
                            continue; 
                        }

                        // D. CETAK TAGIHAN BARU (Hanya dieksekusi jika benar-benar belum ada)
                        logger.info(`[ALGOJO] ??? Menerbitkan tagihan siklus ${cycle.month}-${cycle.year} untuk ${customer.name}`);
