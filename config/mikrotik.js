...
async function connectToMikrotik(nas = null) {
    ...
    try {
        ...
        if (errMsg.includes('!empty') || errMsg.includes('timeout') || errMsg.includes('socktmout')) {
            // Jika router ID ini bikin macet antrean, hampir pasti dia v7!
            // Kita tandai v7 berdasarkan ID-nya supaya antrean scan 247 perangkat lancar
            await pool.query("UPDATE nas SET mikrotik_version = 'v7' WHERE id = ?", [nas.id]);
            nas.mikrotik_version = 'v7';
...
async function getMikrotikConnection(nasData = null) {
    ...
    // Fallback: Jika kosong, coba konek ke router default
...
async function disconnectHotspotUser(username) {
    try {
        ...
        const [session] = await dbPool.query(`
            SELECT r.nasipaddress, n.secret, n.mikrotik_version, r.framedipaddress, r.callingstationid 
            FROM radacct r 
        ...
        if (session.length > 0) {
            ...
            // Eksekusi tendangan Radius (CoA)
            exec(command);
            logger.info(`?? [CoA-KICK] User ${username} ditendang via Radius dari ${nasipaddress} (v7 Friendly)`);
        ...
        const conn = await getMikrotikConnection();
        if (!conn) {
            // Jika Radius sudah jalan tapi API gagal, kita anggap sukses saja
            return session.length > 0 ? { success: true } : { success: false, message: 'Koneksi API gagal' };
        }
        
        const activeUsers = await conn.write('/ip/hotspot/active/print', ['?user=' + username]);
        
        if (activeUsers && activeUsers.length > 0) {
            await conn.write('/ip/hotspot/active/remove', ['=.id=' + activeUsers[0]['.id']]);
            logger.info(`? [API-KICK] Disconnected hotspot user: ${username}`);
        }
...
async function syncAllRouterVersions() {
    try {
        logger.info('?? [SYSTEM] Memulai pemindaian identitas semua router di database...');
        const [allNas] = await pool.query("SELECT * FROM nas");
        
        for (const nas of allNas) {
            logger.info(`?? [SCAN] Mencoba jabat tangan dengan: ${nas.shortname} (${nas.nasname})`);
            // Panggil fungsi koneksi dengan parameter nas lengkap
            const conn = await connectToMikrotik(nas);
            if (conn) {
                await conn.close(); // Tutup lagi setelah kenalan
            }
        }
        logger.info('? [SYSTEM] Pemindaian selesai. Database seharusnya sudah sinkron.');
    } catch (err) {
